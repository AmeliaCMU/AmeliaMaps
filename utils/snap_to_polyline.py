...
class TrajectoryProcessor(MapProcessor):
    def __init__(self, ipath: str, opath: str, airport: str, parrallel: bool, data_to_process: int = 0.05):
    ...
    def process_file_sequential(self, file, out_path_set, save = True):
        ...
        if save: data.to_csv(out_file, index=False)
    ...
    def snap_to_graph(self):
        ...
        for i in tqdm(range(0,data_length)):
            if(self.parrallel): self.process_file_parrallel(self.TRAJECTORY_FILES[i], out_path_set)
            else: self.process_file_sequential(self.TRAJECTORY_FILES[i], out_path_set)

if __name__ == '__main__':
    from argparse import ArgumentParser
    parser = ArgumentParser()
    parser.add_argument('--ipath', default='./boeing/swim/', type=str, help='Input path.')
    parser.add_argument('--opath', default='./boeing/airports/context', type=str, help='Output path.')
    ...
    parser.add_argument('--data-to-process', default=0.05, type=float, help='Decimal percentage of dataset to process')
